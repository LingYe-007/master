...
% twoside：与 \documentclass[twoside] 一致；left/right 在双面下对应内侧/外侧（对称 2.5cm）
% 上边距：减小 top + headsep，避免“页眉实际用 eso-pic 画、版心仍留一大块 head 区域”导致正文起点过低
% 装订：若学院对订口有毫米级要求，可追加 bindingoffset=5mm 等并微调 left/right
\geometry{a4paper, twoside, left=2.5cm, right=2.5cm, top=22mm, bottom=2.5cm, headheight=10pt, headsep=8mm}
\input{images/option}
...
\hypersetup{pdfpagelayout=TwoPageLeft}% 阅读器双页预览时左右装订边一致（首页为奇数右页）
...
% 二级「1. 2. …」：收窄编号盒子与左边距，使数字与正文更贴近（避免编号与文字间大空挡）
\setlist[enumerate,2]{
  label=\arabic*.,
  leftmargin=2.1em,
  labelwidth=1.65em,
...
\newcommand{\cnitem}{%
  ...
  （\arabic{cnparenum}）\hspace{0.12em}%
  \ignorespaces
}
...
% 策略：双面下奇偶页的版心左起点分别为 1in+\oddsidemargin / 1in+\evensidemargin，须与之一致
%       宽度 \textwidth；页眉文字与页眉线置于该区域内并居中
\AddToShipoutPictureFG{%
  \ifnum\value{usefancyheader}=1
    \AtPageUpperLeft{%
      \put(\ifodd\value{page}\dimexpr1in+\oddsidemargin\relax\else\dimexpr1in+\evensidemargin\relax\fi,-\dimexpr9mm\relax){\makebox[\textwidth][c]{\headerfont\ifodd\value{page}\leftmark\else 西北大学硕士专业学位论文\fi}}%
      \put(\ifodd\value{page}\dimexpr1in+\oddsidemargin\relax\else\dimexpr1in+\evensidemargin\relax\fi,-\dimexpr14mm\relax){\rule{\textwidth}{0.4pt}}%
    }%
  \fi
}
...
\fancypagestyle{fancy}{%
  ...
  \fancyhead{}
  \renewcommand{\headrulewidth}{0pt}
  % 页脚居中（与学院 Word 版式常见要求一致）；若需「奇右偶左」可改为 [RO,LE]
  \fancyfoot[C]{\wuhao\rmfamily\thepage}
}
...
% 章首节落在奇数页：偶数页起章前插入一页空白（\thispagestyle{empty}，双面与单面逻辑相同）
...
% 若仍超 3 页可改为 2：目录中不显示「条」级（subsubsection），通常可再省约半页
...
% 标题区竖向间距与「主目录」最初规范一致（1.5em + 1em），不受正文目录 compact 预设影响
\par\addvspace{1.5em}
\begin{center}
\sanhao\heiti 图~~~~目~~~~录
\end{center}
...
% 3. 攻读硕士学位期间取得的科研成果（含文末空白页，盲审/非盲审版均排版；封底不再依赖 Word 截取）
