...
\usepackage{url}
% 正文引用只显示文献编号，右上角上标；参考文献表按 GB/T 7714-2015 顺序编码制（纯数字标号）
% 使用 style=gb7714-2015 统一为顺序编码制；sorting=none 按引用顺序分配数字
\usepackage[backend=biber,style=gb7714-2015,sorting=none,gbpub=false,
gbpunctin=false,gbnamefmt=lowercase,gbbiblabel=bracket,
]{biblatex}
% 强制按引用顺序分配数字标号，避免部分条目使用字母标号
\ExecuteBibliographyOptions{sorting=none}
% 引用为右上角上标格式，带方括号 [1] [2]，仅输出 labelnumber
\DeclareCiteCommand{\cite}[\mkbibbracketsuper]
  {\usebibmacro{citeindex}}
  {\usebibmacro{citeindex}%
   \printfield{labelnumber}}
  {\multicitedelim}
  {}
\newcommand*{\mkbibbracketsuper}[1]{\textsuperscript{[#1]}}
