...
本文以跨视图语义融合与结构化模型压缩为切入点，提出属性-结构双视图对比学习机制以应对联邦推荐中的用户冷启动问题，并将语义感知压缩策略应用于异构图模型，在冷启动、Non-IID 数据适配及端侧轻量化等方面构建了高效、保护隐私、可落地的联邦异构图推荐框架，在算法与系统两个层面给出完整方案。在 MovieLens-1M、Yelp 和 ACM 三个公开数据集上，与 FedNCF、FedGNN、FedPerGNN、FedHGNN、FedProto 等五种基线方法对比，采用 HR@K 与 NDCG@K（命中率 Hit Rate at K、归一化折损累积增益 Normalized Discounted Cumulative Gain at K）作为评估指标，所提模型在 HR@10、NDCG@10 等指标上均优于基线。主要工作包括：
