...
（2）联邦训练监控模块：为管理员提供实时训练状态看板，包括 Loss 曲线、AUC 指标及客户端在线状态，便于发现训练异常、评估模型收敛情况并做出调度决策。
...
\begin{figure}[H]
    ...
    \centering
    \small
    输入：用户请求 + 本地交互历史\\[0.3em]
    \textbf{是否有足够交互？}\quad $\Rightarrow$\quad
    \fbox{是：结构优先}\\[-0.1em]
    \quad 结构视图聚合（用户-论文-会议等）$\rightarrow$ 用户嵌入 $\rightarrow$ Top-$N$ 检索\\[0.4em]
    \textbf{否则}\quad $\Rightarrow$\quad
    \fbox{否：属性优先}\\[-0.1em]
    \quad 属性映射分支（兴趣/机构等）$\rightarrow$ 投影到全局原型空间 $\rightarrow$ 语义相似检索\\[0.4em]
    输出：个性化论文列表 + 元路径注意力解释（可选）
    \end{minipage}}
...
（1）系统监控看板测试。联邦训练状态监控面板实时展示参与训练的客户端数量、当前训练轮次、全局 Loss 与 AUC 等指标随轮次的变化曲线及各客户端在线状态。测试表明，曲线随训练轮次动态更新且趋势收敛，满足监控与调度需求。监控面板如图~\ref{fig:ui_monitor} 所示。
...
（2）通信开销对比测试。在相同数据集与训练轮次下，对比语义感知压缩开启前后端云之间的累计通信流量。结果表明，开启动态元路径选择与残差梯度量化后，通信开销降低约一个数量级，其趋势与第四章表~\ref{tab:communication_cost} 的结论一致，验证了压缩模块在系统集成后的可行性。
