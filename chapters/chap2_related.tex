...
\addcontentsline{toc}{section}{第二章~~相关理论和技术}
...
\subsection{推荐系统基础}
随着信息技术的飞速发展，互联网数据呈现指数级增长趋势，用户面临着从海量数据中筛选感兴趣内容的巨大挑战，即“信息过载”问题。推荐系统（Recommender System, RS）作为解决这一问题的核心技术手段，已广泛应用于电子商务、社交媒体及内容分发平台\cite{zhang2024social}。
...
在此视角下，推荐任务转化为\textbf{链接预测}问题：即根据图中已有的拓扑结构和节点特征，预测在用户节点 $u$ 和物品节点 $v$ 之间是否存在一条潜在的边（Link），并计算其连接概率 $p(u, v)$。根据链接概率来判断用户和物品真实存在关系的可能\cite{he2020lightgcn}。

综上所述，无论是预测矩阵评分 $\hat{y}_{uv}$ 还是预测连接概率 $p(u, v)$，最终系统都会根据预测值对物品进行 Top-K 排序并推荐给目标用户。
...
\begin{enumerate}[wide, labelwidth=!, labelindent=\parindent]
    ...
    \begin{itemize}[wide, labelwidth=!, labelindent=\parindent]
        ...
        \begin{figure}[htbp]
            \centering
            % 请替换为你的实际图片文件名，例如 images/usercf_demo.png
            \includegraphics[width=0.6\textwidth]{images/usecf.png} 
        ...
        \begin{figure}[htbp]
            \centering
            % 请替换为你的实际图片文件名
            \includegraphics[width=0.6\textwidth]{images/CF.png}
    ...
    \textbf{优缺点分析：} 协同过滤算法具有模型通用性强、无需领域知识等优点。然而，其严重依赖历史交互数据，在面对\textbf{数据稀疏（Data Sparsity）}问题时推荐质量会显著下降；同时，对于无历史行为的新用户或新物品，存在严重的\textbf{冷启动（Cold Start）}问题\cite{qiao2018coldstart}。
    ...
    随着深度学习技术的发展，神经网络被广泛应用于提升推荐系统的特征表达能力与泛化能力。常见的方法包括：
    \begin{itemize}
...
\begin{equation}
    ...
    \label{eq:gnn_update}
\end{equation}
通过堆叠 $L$ 层 GNN 网络，节点 $u$ 的最终表示 $\mathbf{e}_u^{(L)}$ 实际上融合了其 $L$ 跳（L-hop）邻居的信息。这种机制使得模型能够显式地编码高阶协同信号（High-order Connectivity）。例如，通过“用户A-物品1-用户B-物品2”的路径，模型可以挖掘出具有相似偏好的潜在用户群体，从而有效缓解数据稀疏性问题，提升推荐的准确性\cite{wang2019kgat}。

\begin{figure}[htbp]
    \centering
    % 请替换为你的实际图片文件名，例如 images/图神经网络.png
    \includegraphics[width=1\textwidth]{images/图神经网络.png} 
...
\begin{itemize}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.3\textwidth]{images/同构图.png} 
        \caption{同构图示意图}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{images/元路径.png}
    \caption{异构图与元路径示意图}
    \label{fig:meta_path}
\end{figure}
...
联邦学习（Federated Learning, FL）是由谷歌（Google）于 2016 年首次提出的一种新兴分布式机器学习范式，旨在打破“数据孤岛”并有效保护用户隐私\cite{li2025openfgl}。

其核心理念在于“\textbf{数据不动模型动}”，即在不共享原始数据、保护数据隐私的前提下，利用分布在不同客户端（Clients）的数据协同训练出一个全局模型。联邦学习通常采用\textbf{客户端-服务器（Client-Server）}架构\cite{deng2023survey}。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{images/联邦推荐图.jpg} 
    \caption{联邦学习架构示意图}
...
\begin{itemize}
    ...
    PTQ 是在模型训练完成并收敛后进行的。它通常只需要少量的校准数据（Calibration Data）来计算量化参数（如缩放因子和零点），无需对整个模型进行重新训练。该方法实施简单、计算成本低，但在极低比特（如 4-bit 以下）场景下可能会导致较大的精度下降\cite{zeng2025feddt}。
...
\end{itemize}
模型剪枝侧重于减少参数数量，而模型量化侧重于降低参数精度。在实际部署中，这两种技术通常是正交且互补的，常被组合使用（例如“先剪枝后量化”）以在保证精度的前提下最大化模型的压缩比\cite{openreview2024fedcomloc}。