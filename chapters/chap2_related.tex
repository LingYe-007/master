...
本章将说明与 FedASCL 框架及语义感知压缩策略直接相关的理论与技术。首先，介绍推荐任务建模、特征表示与预处理方法以及常用评价指标；其次，详述图神经网络推荐中的同构图、异构图、元路径与超图建模；然后，概述联邦学习、对比学习与原型对齐机制；最后，介绍模型剪枝、量化及语义感知压缩策略。
...
\begin{enumerate}[wide, labelwidth=!, labelindent=\parindent]
    \item \textbf{用户冷启动（User Cold Start）}：当新用户首次使用推荐系统时，系统难以获取该用户的历史交互记录，因而难以准确理解其兴趣偏好。在传统集中式推荐系统中，通常可通过用户注册信息（如年龄、性别、职业、地理位置等）或相似用户偏好进行推荐；但在联邦推荐场景下，新用户数据驻留本地，服务器无法利用全局用户信息进行相似度匹配，使得用户冷启动问题更加严峻\cite{zhang2024ifedrec}。
    \item \textbf{物品冷启动（Item Cold Start）}：当新物品加入系统时，由于缺乏用户对该物品的交互记录，系统难以判断其特征与潜在受众。物品冷启动在内容推荐、新闻推荐等场景中尤为突出，因为新内容需要快速获得推荐机会才能被用户发现。
    \item \textbf{系统冷启动（System Cold Start）}：当一个新的推荐系统刚上线时，系统整体缺乏用户与物品的交互数据，难以建立有效的推荐模型。系统冷启动通常需要借助内容特征、外部知识库或迁移学习等方法进行初始化。
\end{enumerate}
...
\textbf{（1）同构图（Homogeneous Graph）}：图中所有顶点属于同一类型，所有边也属于同一类型，即 $G=(V, E)$ 中任意 $u,v \in V$ 类型相同、任意边类型相同。同构图示例如图~\ref{fig:homo_graph}。2.1 节中的用户-物品二部图含有用户与物品两类节点，可视为异构图的一种特例。
...
联邦学习是由谷歌（Google）于 2016 年首次提出的一种分布式机器学习方式\cite{konecny2016federated}，在不集中原始数据的前提下协同训练，并保护用户隐私。McMahan 等人提出的 FedAvg 算法\cite{mcmahan2017fedavg}基于迭代模型平均，确立了联邦学习的基本训练范式。其基本理念在于"\textbf{数据不动模型动}"，即在不共享原始数据、保护数据隐私的同时，利用分布在不同客户端（Clients）的数据协同训练出一个全局模型；联邦学习通常采用\textbf{客户端-服务器（Client-Server）}架构\cite{deng2023survey}。

\subsubsection{联邦训练流程}
一轮标准的联邦学习训练通常包含以下四个步骤：

\begin{enumerate}
    \item \textbf{本地训练}：客户端接收服务器下发的全局模型参数，利用本地数据进行若干步优化，得到本地模型参数。

    \item \textbf{参数上传}：客户端将本地模型参数或参数更新上传至服务器。该过程可结合加密或安全聚合协议，避免泄露单个客户端的信息。

    \item \textbf{聚合更新}：服务器对参与客户端的上传结果进行聚合，更新全局模型。

    \item \textbf{模型下发}：服务器将更新后的全局模型发送回客户端，客户端用其初始化下一轮训练。
\end{enumerate}
...
最常用的聚合算法是加权平均聚合 FedAvg，其基本思路是根据各客户端的数据量对模型参数进行加权平均。设第 $t$ 轮训练中，参与训练的客户端集合为 $\mathcal{M}^{(t)}$，客户端 $m$ 的本地模型参数为 $\Theta_m^{(t)}$，数据量为 $|\mathcal{D}_m|$，则全局模型参数的更新公式为：
\begin{equation}
...
为增强隐私保护，可对上传的参数更新或原型添加\textbf{差分隐私噪声}\cite{zhang2024fedtgp}。

\textbf{差分隐私（Differential Privacy）}的基本思路是：在查询结果（如梯度、原型向量）上注入随机噪声，使得单条记录的加入或删除对输出分布的影响被严格限制在可量化范围内；该范围由隐私预算 $\varepsilon$ 控制，$\varepsilon$ 越小则隐私保护越强。因此，攻击者难以从聚合结果反推个体数据。

\textbf{拉普拉斯机制（Laplace Mechanism）}是常用的实现方式。对数值型输出 $f(D)$ 添加服从拉普拉斯分布的噪声 $\xi \sim \text{Laplace}(0, \Delta/\varepsilon)$，其中 $\Delta$ 为函数 $f$ 的全局敏感度（单条记录变化引起的最大输出变化），$\varepsilon$ 为隐私预算。拉普拉斯机制对应 $\delta=0$ 的 $\varepsilon$-差分隐私；若采用\textbf{高斯机制}等变体，则可满足更一般的 $(\varepsilon,\delta)$-差分隐私。需注意的是，$\varepsilon$ 越小隐私越强，但噪声越大，模型精度通常有所下降，实践中需在隐私与效用之间权衡。

对上传的模型参数或原型施加上述噪声，可在满足差分隐私约束的前提下防御推理攻击与后门攻击。
...
在推荐系统中，跨视图对比学习通过最大化不同视图间的互信息实现语义对齐与知识迁移。联邦推荐场景下常见视图包括：\textbf{（1）属性视图}，基于用户和物品属性构建的语义视图（如 k-NN 超图）；\textbf{（2）结构视图}，基于用户-物品交互历史构建的交互结构视图。将同一节点在两种视图下的表示视为正样本对，通过 NT-Xent 等对比损失最大化互信息，把属性语义迁移到交互空间，适用于交互稀疏场景。FedASCL 的核心即采用属性-结构双视图对比学习，在客户端本地执行，无需上传原始数据，满足联邦隐私约束\cite{crad2024multiview}。
...
联邦原型学习是原型学习在联邦学习场景下的扩展。其基本流程包括：\textbf{（1）本地原型计算}，客户端基于当前的节点嵌入表示，计算属于各个语义类别的节点嵌入均值作为本地原型；\textbf{（2）全局原型聚合}，服务器收集各客户端上传的本地原型，采用加权平均策略聚合生成全局语义原型；\textbf{（3）隐私保护}，客户端在上传前可对原型添加噪声以满足差分隐私（见\ref{sec:fl_privacy}节）\cite{zhang2024fedtgp}。通过原型对比损失（Prototype-based Contrastive Loss），客户端约束本地节点嵌入尽可能靠近其所属类别的全局原型，同时远离其他类别的原型，从而实现跨客户端的语义对齐。图~\ref{fig:fed_prototype_align} 给出了联邦原型聚合与语义对齐的流程示意。
...
传统剪枝与量化多为语义无关，而异构图推荐中模型参数往往具有明确语义（如元路径对应不同语义通道），粗暴压缩易破坏关键语义、导致性能下降。语义感知压缩在压缩过程中保留对任务最重要的语义信息，需要：\textbf{（1）语义重要性评估}，基于注意力权重或梯度评估各语义通道重要性；\textbf{（2）动态语义选择}，按重要性保留语义通道；\textbf{（3）语义完整性保证}，压缩后仍保持关键语义结构以维持推荐性能。
...
本文第四章提出的压缩策略即采用上述语义感知思路：动态元路径选择器基于 FedASCL 的注意力权重评估各元路径重要性并剔除冗余通道，实现结构级剪枝；残差梯度量化对保留的梯度进行低比特量化并辅以本地误差累积，在约 20 倍压缩下保持推荐精度\cite{semfed2024}。
