...
联邦学习是由谷歌（Google）于 2016 年首次提出的一种分布式机器学习方式，在不集中原始数据的前提下协同训练，并保护用户隐私\cite{li2025openfgl}。其基本理念在于"\textbf{数据不动模型动}"，即在不共享原始数据、保护数据隐私的同时，利用分布在不同客户端（Clients）的数据协同训练出一个全局模型；联邦学习通常采用\textbf{客户端-服务器（Client-Server）}架构\cite{deng2023survey}。
...
为增强隐私保护，可对上传的参数更新或原型添加\textbf{差分隐私噪声}\cite{zhang2024fedtgp}。差分隐私（Differential Privacy）通过在查询结果（如梯度、原型向量）上注入随机噪声，使得单条记录的加入或删除对输出分布的影响被严格限制在可量化范围内（由隐私预算 $\varepsilon$ 控制），从而难以从聚合结果反推个体数据。\textbf{拉普拉斯机制（Laplace Mechanism）}是常用的实现方式：对数值型输出 $f(D)$ 添加服从拉普拉斯分布的噪声 $\xi \sim \text{Laplace}(0, \Delta/\varepsilon)$，其中 $\Delta$ 为函数 $f$ 的全局敏感度（单条记录变化引起的最大输出变化），$\varepsilon$ 为隐私预算；$\varepsilon$ 越小隐私保护越强，但噪声越大、模型精度通常有所下降。对上传的模型参数或原型施加此类噪声，可在满足 $\varepsilon$-差分隐私（拉普拉斯机制对应 $\delta=0$ 的情形）或更一般的 $(\varepsilon,\delta)$-差分隐私（如高斯机制）的前提下防御推理攻击与后门攻击。
