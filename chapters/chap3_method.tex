...
\subsection{算法总体框架} 
\label{sec:framework}
FedASCL算法遵循标准的"服务器-客户端"联邦训练范式，其整体架构由五大核心模块构成：属性语义视图构建、跨视图对比学习、基于原型的语义对齐、基于元路径的推荐以及隐私保护机制。算法的端到端执行流程如下：
...
\begin{enumerate}
    ...
    \item \textbf{细粒度跨视图对比}：在客户端本地执行跨视图对比学习，用于最大化属性语义表示与交互结构表示之间的互信息，完成语义迁移与增强\cite{crad2024multiview}；
    ...
    \item \textbf{联合优化与推荐}：客户端利用预定义的元路径挖掘异构图中的高阶语义关联，结合推荐主任务损失与对比学习辅助损失进行联合反向传播，完成模型参数的闭环更新。
\end{enumerate}
...
实验选取了三个具有丰富属性信息的代表性数据集：\textbf{MovieLens-1M}、\textbf{Yelp} 和 \textbf{ACM}。具体统计信息如表 \ref{tab:dataset_stat} 所示。
...
\begin{enumerate}
    \item \textbf{稀疏场景优势明显}：在数据最稀疏的 Yelp 数据集上，FedASCL 相比次优模型 FedProto 在 HR@10 和 NDCG@10 上分别提升了 \textbf{2.88\%} 和 \textbf{3.36\%}。这说明引入属性视图能够弥补交互数据的匮乏。
    \item \textbf{性能提升稳定}：在 MovieLens-1M 数据集上，FedASCL 相比 FedProto 在 HR@10 和 NDCG@10 上分别提升了 \textbf{1.42\%} 和 \textbf{1.52\%}；在 ACM 数据集上，提升幅度分别为 \textbf{2.35\%} 和 \textbf{2.53\%}，说明了方法在不同数据集上的鲁棒性。
    \item \textbf{优于传统 GNN}：相比于 FedHGNN，FedASCL 通过引入跨视图对比学习，挖掘了属性与结构间的互信息，使得节点表征更具判别力。
\end{enumerate}
...
\subsection{本章小结}
本章通过多维度的实验验证了 FedASCL 的优越性。实验结果表明，该算法不仅在总体性能上超越了基线模型，更在冷启动和 Non-IID 场景下表现出极强的鲁棒性，解决了联邦推荐系统面临的核心挑战。