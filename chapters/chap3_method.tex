...
第二章介绍了推荐系统、图神经网络、联邦学习及对比学习等理论基础。本章针对第一章提出的冷启动与数据异质性问题，提出 FedASCL 框架，先说明问题背景与动机，再介绍算法总体框架及各核心模块，最后通过实验验证有效性。
...
针对上述挑战，本章提出基于属性-结构双视图对比学习的联邦异构图推荐算法（Federated Attribute-Structure Contrastive Learning, FedASCL）。算法通过双视图对比学习将属性语义迁移至交互空间，并引入基于原型的语义对齐以矫正 Non-IID 带来的分布偏差，在保护隐私的前提下提升冷启动场景下的推荐质量\cite{wang2025fedpcl}。
...
为实现从属性域到交互域的知识迁移，本章设计了细粒度的跨视图对比学习机制：基于互信息最大化，使同一节点在“属性视图”与“结构视图”下的表示在共享嵌入空间中保持语义一致\cite{wang2025fedpcl}，包含双视图编码与对比优化两个阶段。
...
本实验选取了三类主流算法作为对比基准：经典联邦推荐 FedNCF\cite{jiang2022fedncf}，联邦图神经网络 FedGNN\cite{wu2021fedgnn}、FedPerGNN\cite{wu2022fedpergnn}、FedHGNN\cite{yan2024fedhgnn}，以及联邦原型学习 FedProto\cite{tan2022fedproto}。
...
从表~\ref{tab:non_iid_data} 可以看出，当数据分布极端倾斜（$\alpha=0.1$）时，FedASCL 的性能衰减最小（仅 6.9\%），明显优于 FedGNN 的 25.1\% 和 FedHGNN 的 18.1\%。此外，FedASCL 在所有 $\alpha$ 值下均保持了最优性能，说明了全局语义原型能够校正本地模型的语义漂移，使模型在不同数据分布下都具有良好的鲁棒性。
