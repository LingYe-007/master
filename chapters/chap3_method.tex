...
\begin{cnenumerate}
    \cnitem \textbf{客户端本地双视图构建。}客户端首先基于用户和物品的原始属性构建属性语义图，同时利用本地交互历史构建交互结构图，形成互补的双视图输入。
    \cnitem \textbf{双视图特征编码。}利用异构图神经网络分别对结构视图和属性视图进行编码，学习节点在不同视角下的高维特征表示。
    \cnitem \textbf{细粒度跨视图对比。}在客户端本地执行跨视图对比学习，用于最大化属性语义表示与交互结构表示之间的互信息，完成语义迁移与增强\cite{crad2025multiview}。
    \cnitem \textbf{原型语义对齐。}服务器聚合生成全局语义原型并下发，客户端利用这些原型对本地节点表示进行正则化约束，以缓解 Non-IID 导致的模型偏差；与通过全局知识引导融合本地嵌入的联邦推荐框架在跨客户端对齐目标上相通\cite{lim2025fedrkg}。
    \cnitem \textbf{模型聚合与更新。}服务器收集各客户端上传的模型参数（或梯度），采用加权平均策略聚合后更新全局模型。
    \cnitem \textbf{联合优化与推荐。}客户端利用预定义的元路径挖掘异构图中的高阶语义关联，结合推荐主任务损失与对比学习辅助损失进行联合反向传播，完成模型参数的闭环更新。
\end{cnenumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{images/fedascl_framework.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/属性语义图构建.png}
...
\begin{cnenumerate}
    \cnitem {\songti\bfseries 结构视图编码。}基于用户-物品交互图 $\mathcal{G}_{stru}$，利用异构图卷积层聚合邻居信息。设 $\mathbf{h}_i^{(l)}$ 为节点 $i$ 在第 $l$ 层的隐藏状态，其更新规则为：
    \begin{equation}
    ...
    \cnitem \textbf{属性视图编码。}基于第\ref{subsec:attr_sem_graph}节构建的属性语义超图 $\mathcal{G}_{attr}$（对应关联矩阵 $\mathbf{H}$），采用超图卷积网络进行编码：
    \begin{equation}
...
为了对齐 $\mathbf{z}_i^{stru}$ 和 $\mathbf{z}_i^{attr}$，我们将同一节点在两个视图下的嵌入视为{\songti\bfseries 正样本对}，将该节点与其他节点在任意视图下的嵌入视为{\songti\bfseries 负样本对}。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/跨视图对比机制.png}
...
该策略的作用主要体现在两个方面：\textbf{纠偏}层面上，即便本地数据分布极度倾斜（例如全是动作片），全局原型仍能指示其他语义（如"爱情片"）在嵌入空间中的大致位置，从而防止本地模型为拟合局部数据而破坏整体语义结构；\textbf{冷启动增强}层面上，新接入客户端往往缺乏交互、初始嵌入随机或由属性粗估，全局原型为其提供高质量参照，使其快速对齐全局分布。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/基于原型的语义对齐策略.png}
...
元路径 $\Phi$ 定义为连接网络模式中节点类型的复合关系序列，即 $A_1 \xrightarrow{R_1} A_2 \xrightarrow{R_2} \dots \xrightarrow{R_l} A_{l+1}$。在推荐场景中，不同的元路径代表了不同的语义偏好。本章主要考虑两类元路径并组成集合 $\mathcal{P} = \{\Phi_1, \Phi_2, \dots, \Phi_P\}$：\textbf{协同关联路径}（如 U-I-U，刻画购买过相同物品的用户之相似偏好）与\textbf{属性关联路径}（如 U-A-U、I-C-I，借助属性图语义连接弥补交互稀疏\cite{li2023metapath}）。
...
在每一轮训练的前向传播中，时间开销主要来自图卷积：\textbf{结构视图}在用户–物品二部图 $\mathcal{G}_{stru}$ 上做消息传递，稀疏实现下单层与边数线性相关即 $O(M \cdot d)$，$L$ 层合计 $O(L \cdot M \cdot d)$；\textbf{属性视图}基于超图关联矩阵 $\mathbf{H}$，每节点连 $k$ 个邻居使非零元约为 $N(k+1)$，$\mathbf{H}\mathbf{H}^\top \mathbf{X}$ 一类操作主导复杂度约 $O(L \cdot N \cdot k \cdot d)$。
由于 $k$ 通常较小（实验中设为 5--10），且 $M \gg N$，属性视图的引入并未明显增加图传播的数量级。

联合优化阶段，\textbf{跨视图对比损失} $\mathcal{L}_{cl}$ 若在全图计算 InfoNCE 为 $O(N^2 d)$，不可行；本章采用批内训练策略，仅在批量 $B$ 内计算相似度，降为 $O(B^2 d)$，且 $B \ll N$ 时开销很低。\textbf{原型对齐损失} $\mathcal{L}_{proto}$ 需与 $K$ 个全局原型比对，为 $O(B \cdot K \cdot d)$（$K$ 为常数级语义类数）。\textbf{主任务 BPR 损失} $\mathcal{L}_{rec}$ 仅涉正负样本对内积，为 $O(B \cdot d)$。
...
\begin{enumerate}[label=\arabic*., labelsep=0.28em, leftmargin=2.45em, noitemsep, topsep=2pt, parsep=0pt]
    \item \textbf{RQ1（总体性能）}FedASCL 在常规联邦推荐场景下，是否优于现有的基线方法？
    \item \textbf{RQ2（冷启动适应性）}在缺乏交互历史的冷启动场景下，双视图机制能否提升推荐质量？
    \item \textbf{RQ3（鲁棒性与消融）}面对 Non-IID 数据分布，算法是否具有鲁棒性？各关键模块（对比学习、原型对齐）贡献如何？
\end{enumerate}

\subsubsection{数据集与预处理}
实验选取了三个具有丰富属性信息的代表性数据集，即 \textbf{MovieLens-1M}、\textbf{Yelp} 和 \textbf{ACM}，具体统计信息见表~\ref{tab:dataset_stat}。
...
\begin{table}[H]
    ...
    \begin{tabular}{l|cc|cc|cc}
        ...
        \bottomrule
    \end{tabular}}
    {\wuhao\songti 注：$^*$ 表示该数据集上本方法取得最优结果（表~\ref{tab:exp_config_chap3} 中为固定随机种子的单次运行，未另行报告假设检验）。}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{images/experiment.png}
    \caption{各算法训练过程性能对比（100轮训练）}
    \label{fig:training_progress}
\end{figure}

图~\ref{fig:training_progress} 展示了各算法在 100 轮训练过程中的性能变化曲线。从图中可以看出，所有算法的性能均随训练轮次逐渐提升，并在约 60--80 轮后趋于稳定。FedASCL 在所有数据集和评估指标上均取得了最优性能，且收敛速度与稳定性均优于其他基线方法。
...
\begin{table}[H]
    \centering
    \caption{不同 Non-IID 程度 ($\alpha$) 下的性能鲁棒性对比（MovieLens-1M，NDCG@10；括号内为相对 $\alpha=\infty$ 的降幅）}
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{FedASCL} & \textbf{0.1182} (-6.9\%) & \textbf{0.1245} & \textbf{0.1268} & \textbf{0.1270} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images/non_iid_robustness.png}
    \caption{不同数据异质性水平下的性能对比（100轮训练）}
    \label{fig:non_iid_robustness}
\end{figure}

从图~\ref{fig:non_iid_robustness} 可见，所有算法在极端异质性（$\alpha=0.1$）下性能均低于 IID 场景（$\alpha=\infty$），随 $\alpha$ 增大（异质性降低）性能逐渐提升；当 $\alpha$ 达到 0.5 后提升趋于稳定，说明降低异质性的收益存在饱和点。

表~\ref{tab:non_iid_data} 显示，当数据分布极端倾斜（$\alpha=0.1$）时，FedASCL 的性能衰减最小（仅 6.9\%），优于 FedGNN 的 25.1\% 和 FedHGNN 的 18.1\%。FedASCL 在所有 $\alpha$ 值下均保持最优性能，说明全局语义原型能够校正本地模型的语义漂移，使模型在不同数据分布下具有较好的鲁棒性。
...
\subsection{本章小结}
本章围绕“总体性能、冷启动适应性与 Non-IID 鲁棒性”三个维度对 FedASCL 进行了实验验证。总体性能方面，在 MovieLens-1M、Yelp 与 ACM 三个数据集上，FedASCL 在 HR@10、NDCG@10 等指标上均优于 FedNCF、FedGNN、FedPerGNN、FedHGNN 与 FedProto 等基线（表~\ref{tab:overall_performance}），说明属性-结构双视图机制能够在联邦约束下更充分地挖掘异构语义并提升排序质量。冷启动方面，在仅保留属性信息、移除交互记录的设定下，FedASCL 仍保持稳定优势（表~\ref{tab:cold_start_res}），体现了属性视图对“零交互”用户的有效补偿。Non-IID 方面，随着 Dirichlet 参数变化，FedASCL 在不同异质性水平下的性能趋势更平稳（图~\ref{fig:non_iid_robustness}），表明原型语义对齐能够缓解客户端漂移带来的分布偏差。进一步的消融实验（表~\ref{tab:ablation}）显示，移除属性视图会带来最明显的性能退化，而移除原型对齐同样会削弱联邦场景下的纠偏能力，验证了各核心模块在整体效果中的作用分工。